...
function buildData (files, newDir, serviceName, scenarioName) {
  var userjourneys = files.filter(function (file) {
    ...
    return {
      'caption': caption,
      'imgref': 'images/' + scenarioName + '/' + image,
      'note': 'Notes go here...'
    }
  })

  return new Promise(function (resolve, reject) {
    fs.readFile(path.join(newDir, 'data.js'), 'utf8', function (err, data) {
      if (err) {
        data = {
          'service': serviceName,
      ...
      fs.writeFile(path.join(newDir, 'data.js'), data, function (err) {
        if (err) {
          reject(new Error('There was a problem creating the data for your new service: ' + newDir))
        }

        var servicePath = path.join(newDir, 'index.html')
